...
func TestUpdateRemoteContractCompoundSettings(t *testing.T) {
	net := network.New(t)
...
func TestDeleteRemoteContractCompoundSettings(t *testing.T) {
	net := network.New(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteRemoteContractCompoundSettings(), append([]string{tc.id}, tc.args...))
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
