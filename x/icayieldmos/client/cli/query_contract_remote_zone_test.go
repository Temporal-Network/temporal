...
func TestShowContractRemoteZone(t *testing.T) {
	net, objs := networkWithContractRemoteZoneObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowContractRemoteZone(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetContractRemoteZoneResponse
...
func TestListContractRemoteZone(t *testing.T) {
	net, objs := networkWithContractRemoteZoneObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListContractRemoteZone(), args)
			require.NoError(t, err)
			var resp types.QueryAllContractRemoteZoneResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
