...
func TestShowRemoteContractCompoundSettings(t *testing.T) {
	net, objs := networkWithRemoteContractCompoundSettingsObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		args []string
		err  error
		obj  types.RemoteContractCompoundSettings
	}{
		{
			desc: "found",
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowRemoteContractCompoundSettings(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetRemoteContractCompoundSettingsResponse
...
func TestListRemoteContractCompoundSettings(t *testing.T) {
	net, objs := networkWithRemoteContractCompoundSettingsObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListRemoteContractCompoundSettings(), args)
			require.NoError(t, err)
			var resp types.QueryAllRemoteContractCompoundSettingsResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
