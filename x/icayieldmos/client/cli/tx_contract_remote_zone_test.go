...
func TestCreateContractRemoteZone(t *testing.T) {
	net := network.New(t)
...
func TestUpdateContractRemoteZone(t *testing.T) {
	net := network.New(t)
...
func TestDeleteContractRemoteZone(t *testing.T) {
	net := network.New(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteContractRemoteZone(), append([]string{tc.id}, tc.args...))
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
