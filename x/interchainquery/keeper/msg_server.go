...
import (
	...
	// TODO clean up dep
	//"github.com/temporal-zone/temporal/utils"
	"github.com/temporal-zone/temporal/x/interchainquery/types"
)
...
func (k msgServer) SubmitQueryResponse(goCtx context.Context, msg *types.MsgSubmitQueryResponse) (*types.MsgSubmitQueryResponseResponse, error) {
	...
	// check if the response has an associated query stored on temporal
	...
	if err != nil {
		k.Logger(ctx).Error(query.ChainId, query.CallbackId, "QUERY PROOF VERIFICATION FAILED - QueryId: %s, Error: %s", query.Id, err.Error())
		return nil, err
	}
	...
	if query.Ttl < currBlockTime {
		k.Logger(ctx).Error(query.ChainId, query.CallbackId, "QUERY TIMEOUT - QueryId: %s, TTL: %d, BlockTime: %d", query.Id, query.Ttl, ctx.BlockHeader().Time.UnixNano())
		return &types.MsgSubmitQueryResponseResponse{}, nil
	}

	// If the query is contentless, end
	if len(msg.Result) == 0 {
		k.Logger(ctx).Info(query.ChainId, query.CallbackId, "Query response is contentless - QueryId: %s", query.Id)
		return &types.MsgSubmitQueryResponseResponse{}, nil
	}
	...
	if err != nil {
		k.Logger(ctx).Error(query.ChainId, query.CallbackId,
